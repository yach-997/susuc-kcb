...
-- 2) 按日报表（需已登录 token）
create or replace function public.admin_day_report(p_token text, p_day date)
returns json
language plpgsql
security definer
...
begin
  ...
  day_start := (p_day::timestamp AT TIME ZONE 'Asia/Shanghai');
...
grant execute on function public.admin_day_report(text, date) to anon, authenticated;
